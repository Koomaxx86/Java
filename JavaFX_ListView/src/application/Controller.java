...
public class Controller implements Initializable {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		listView.setOnMouseClicked(new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent event) {
				...
				if( event.getClickCount() == 2) {
					...
					// sub.fxml를 가져와서 객체 생성후 loader에 저장 
					...
					// Sub.fxml 에 연결된 SubController 를 불러와서 subController 에 저장
					SubController subController =  loader.getController();
					
					if (subController != null)
					// inputItemIndex 메소드 정의
					// subController.inputItemIndex에 위에서 저장한 index값을 전달
		...
		// 리스트뷰 아이템 선택 이벤트
		listView.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<String>() { // 익명구현 객체
