 ...
 *  - peek()	: head의 데이터를 반환, Queue가 비어있으면 null 반환
 *  
 *  요소 삭제
 *  - remove()	: head의 데이터를 삭제하고, 삭제된 값을 반환
 *  - poll()	: head의 데이터를 삭제하고, 삭제된 값을 반환, Queue가 비어있으면 null 반환
 */

public class QueueEx {
public static void main(String[] args) {
	
	// list중에 Queue를 구현할수 있는건 LinkedList 뿐이다.
	Queue<Integer> queue = new LinkedList<Integer>();
	// queue.poll();	: 요소가 없으면  null을 반환해서 예외처리를 하지 않아도 된다.
	// queue.remove();	: 요소가 없을 떄도, 삭제를 시도해서, 예외 발생
	...
	int remove1 = queue.poll();		// 값 1 삭제 후 remove1에 반환
	System.out.println("삭제1 : " + remove1);
	int remove2 = queue.poll();		// 값 2 삭제 후 remove1에 반환
