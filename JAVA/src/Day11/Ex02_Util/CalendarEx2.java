...
public class CalendarEx2 {
	...
	public static void main(String[] args) {
		...
		System.out.println( cal.get(Calendar.MONDAY) + 1 ); // 월 1 추가
		System.out.println( cal.get(Calendar.DAY_OF_MONTH) );

		SimpleDateFormat sdf = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
		String today = sdf.format( cal.getTime() ); // 매개변수로 생성된 객체를 넘겨준다
		System.out.println(today);

		// 10분 후
		cal.add(cal.MINUTE, 10); // add 시간을 추가
		...
		// 추첨일 오후 8시부터 다음일(일요일) 오전 6시까지는 판매중지

		// 조건
		// 일요일 오전 6시 ~ 토요일 오후 8시에 구매 : 이번주 토요일 오후 9시
		Calendar lottoCal = Calendar.getInstance();
		// 날짜 변경
		// lottoCal.set(Calendar.DAY_OF_MONTH, 13); // setter을 통해 날짜 수정 후 테스트
		...
		System.out.println(시간);			// 0 : 오전, 1 : 오후
		...
		// 1. 토요일 이면서, 오후 8시 이후 - 구입불가
		if( todayOfWeek.equals("토") && ampm == 1 && 시간 >= 8) {
			System.out.println("구입불가");
		}

		// 2. 일요일 이면서, 오전 6시 이전 - 구입불가
		else if( todayOfWeek.equals("일") && ampm == 0 && 시간 < 6) {
			System.out.println("구입불가");
		}

		// 3. 일요일 이면서, 오전 6시 이후 - 다음 주 토요일 9시가 추첨일시 [+6일 후]
