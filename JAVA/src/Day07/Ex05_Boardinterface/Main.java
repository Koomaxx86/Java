...
public class Main {
	...
	static int max = 10; 							// 최대 게시글 갯수 제한
	static Text boardList[] = new Board[max]; 		// 게시글 목록
	static Text commentList[] = new Comment[max]; 	// 댓글목록
	static DataService data = new BoardAccess(); 	// 데이터베이스 접근 개체 (게시글)
	...
	public static void list() {
		...
		for (Text board : boardList) { // foreach문
	...
	public static void read() {
		...
		Text board = data.select(boardNo); // 객체 불러오기
