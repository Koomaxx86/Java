...
// 타입지정이 가능한 클래스 : B, C
class D <T extends B> { // 타입 미지정 후 객체생성?
	private T t;
...
// i) Interger 클래스만 타입매개변수로 가능하도록 제한 (효용성 X)
...
public class LimitedTypeClass {
	public static void main(String[] args) {
		D<B> d1 = new D<B>();
		D<C> d2 = new D<C>();
//		D<A> d1 = new D<A>(); // A타입은 타입매개변수로 사용불가
		// D 클래스의 타입 매개변수를 B 클래스로 제한하고 있기 때문에,
		// B 클래스의 자식 클래스인 C만 접근 가능하다.
		
		d1.setT( new B() );
		d2.setT( new C() );

		d1.setT( new C() ); // d1<B> <-- 자식객체인 C 대입 가능 (업캐스팅)
//		d2.setT( new B() ); // d2<C> <-- 부모객체인 B 대입 불가 // 업캐스팅 -> 다운캐스팅이 아니라서 권장하지는 않는다.
