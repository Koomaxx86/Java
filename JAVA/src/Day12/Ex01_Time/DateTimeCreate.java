...
public class DateTimeCreate {
	public static void main(String[] args) {
		...
		LocalDate nowDate = LocalDate.now();			// 현재 날짜 정보를 갖는 객체 생성
		System.out.println("현재 날짜 : " + nowDate);

		LocalDate endDate = LocalDate.of(2024, 1, 24);  // 날짜 정보를 지정해서 갖는 객체 생성
		...
		ZonedDateTime new_YorkDateTime = ZonedDateTime.now( ZoneId.of("America/New_York"));
		System.out.println("뉴욕 세계시 : " + new_YorkDateTime);
		...
		if( ld.isLeapYear()) {
			System.out.println("올래는 윤년 : 2월 29일 까지");
		} else {
			System.out.println("올래는 평년 : 2월 28일 까지");
		}
